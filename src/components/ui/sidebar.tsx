...
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7 * 1000;
...
function SidebarProvider({
	...
	const [_open, _setOpen] = useState(() => {
		...
		if (typeof window !== 'undefined') {
			const cookieValue = getClientCookie(SIDEBAR_COOKIE_NAME);
	...
	const setOpen = useCallback(
		async (value: boolean | ((value: boolean) => boolean)) => {
			...
			await setCookie(SIDEBAR_COOKIE_NAME, openState.toString(), SIDEBAR_COOKIE_MAX_AGE);
		},
		[setOpenProp, open],
	);
